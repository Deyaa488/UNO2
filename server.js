...
// -------------------------------------------------------------
// Helper: Get Local LAN IPv4 Addresses with Smart Interface Prioritization
// Filters out virtual adapters (VirtualBox, VMware, Docker, vEthernet, etc.)
...
function getLocalIPs() {
    ...
    const virtualRegex = /virtual|vbox|vmware|docker|vethernet|tap|tun|wsl|loopback|teredo|npcap|hyper-v|bridge|dummy/i;
    const wifiRegex = /wi-?fi|wlan|wireless|wl/i;
    const ethRegex = /eth|ethernet|en|lan/i;
    ...
    for (const devName in interfaces) {
        ...
        for (const iface of ifaceList) {
            if (iface.family === 'IPv4' && !iface.internal) {
                ...
                const isVirtualSubnet = addr.startsWith('192.168.56.') || // VirtualBox host-only
                                        addr.startsWith('169.254.') ||   // APIPA link-local
                                        addr.startsWith('172.17.') ||    // Docker default
                                        addr.startsWith('172.18.') ||
                                        addr.startsWith('10.0.75.');
                const isVirtualName = virtualRegex.test(devName);
                const isVirtual = isVirtualName || isVirtualSubnet;

                let priority = 50;
                if (isVirtual) {
                    priority = 10;
                } else if (wifiRegex.test(devName)) {
                    priority = 100; // Wi-Fi preferred for mobile LAN play!
                } else if (ethRegex.test(devName)) {
                    priority = 80;  // Physical Ethernet
                } else {
                    priority = 60;
                }
...
const server = http.createServer((req, res) => {
    ...
    // API 1: Network & Server Info
    if (pathname === '/api/info') {
        const localIps = getLocalIPs();
        const primary = localIps.find(i => !i.isVirtual) || localIps[0];
    ...
    // API 2: Discover Active Waiting Rooms (LAN Fast Discovery)
    if (pathname === '/api/rooms') {
        const roomList = [];
        rooms.forEach(r => {
            if (!r.isSolo && r.status === 'waiting') {
                roomList.push({
                    code: r.code,
                    name: r.name,
                    hostNickname: r.players[0] ? r.players[0].nickname : 'المضيف',
...
function create108UnoDeck() {
    ...
    for (let k = 0; k < 4; k++) {
        deck.push({ id: id++, color: "wild", hex: "#111111", val: "🌈", type: "wild" });
        deck.push({ id: id++, color: "wild", hex: "#111111", val: "+4", type: "wild4" });
    }
    return deck;
}
...
// -------------------------------------------------------------
// Room Class & State Management
// -------------------------------------------------------------
class Room {
    constructor(code, hostSessionId, hostNickname, ws, isSolo = false, roomName = '', password = '') {
        this.code = code;
        this.hostSessionId = hostSessionId;
        this.isSolo = isSolo;
        this.name = (roomName || '').trim() || ('غرفة ' + (hostNickname || 'المضيف'));
...
// -------------------------------------------------------------
// WebSocket Rooms Store & Code Generator
// -------------------------------------------------------------
const rooms = new Map();
...
wss.on('connection', (ws) => {
    ...
    function handleClientMessage(ws, msg) {
        ...
        if (type === 'create_room') {
            const { nickname, sessionId, roomName, password } = msg;
            let code = generateRoomCode();
            while (rooms.has(code)) code = generateRoomCode();

            const room = new Room(code, sessionId, nickname, ws, false, roomName, password);
            rooms.set(code, room);
        ...
        if (type === 'create_solo_game') {
            ...
            const room = new Room(code, sessionId, nickname, ws, true, 'لعب فردي ضد البوتات');
            rooms.set(code, room);
            clientSessionId = sessionId;
            clientRoomCode = code;
            ...
            for (let seat = 1; seat < totalSeats; seat++) {
                room.players.push({
                    seatIndex: seat,
                    sessionId: 'bot_' + seat + '_' + Date.now() + '_' + Math.random(),
                    nickname: 'بوت ' + seat,
                    isHost: false,
                    isBot: true,
                    connected: true,
        ...
        if (type === 'join_room') {
            const { roomCode, nickname, sessionId, password } = msg;
            ...
            if (room.password) {
                ...
                if (suppliedPass !== room.password) {
                    return sendError(ws, 'كلمة السر غير صحيحة! تأكد من كلمة مرور الغرفة');
                }
            }
        ...
        if (type === 'reconnect') {
            ...
            const isPendingWildForMe = !!(room.pendingWild && (room.pendingWild.playerSeat === player.seatIndex || room.pendingWild.sessionId === player.sessionId));
            ...
            if (room.status === 'playing') {
                ws.send(JSON.stringify({
                    ...
                    pendingWildForMe: isPendingWildForMe
                }));
        ...
        // 4.B طرد لاعب بواسطة المضيف (Host Kick Player)
        if (type === 'kick_player') {
            if (!player.isHost) return sendError(ws, 'المضيف فقط يملك صلاحية طرد اللاعبين');
            if (room.status !== 'waiting') return sendError(ws, 'يمكن طرد اللاعبين فقط في اللوبي قبل بدء اللعبة');

            const targetSeat = parseInt(msg.seatIndex);
            if (isNaN(targetSeat) || targetSeat === player.seatIndex) {
                return sendError(ws, 'لا يمكنك طرد نفسك');
            }

            const targetPlayer = room.players.find(p => p.seatIndex === targetSeat);
            if (!targetPlayer) return sendError(ws, 'اللاعب المراد طرده غير موجود');
            ...
            const pIdx = room.players.indexOf(targetPlayer);
            if (pIdx > -1) {
            ...
            room.broadcast({
                ...
                kickedNickname: targetPlayer.nickname,
                players: room.getPublicPlayerList()
            });
            return;
        }
        ...
        if (type === 'play_card') {
            ...
            // Wild card played without chosen color -> request color
            ...
            // Normal card or wild card with pre-chosen color
            const validColors = ['red', 'blue', 'green', 'yellow'];
            const finalColor = (card.color === 'wild') ? chosenColor : card.color;
            if (card.color === 'wild' && !validColors.includes(finalColor)) {
                return sendError(ws, 'لون غير صالح تم اختياره لكرت Wild');
            }

            player.cards.splice(cardIndex, 1);
            executeCardPlay(room, player, card, finalColor);
            return;
        }

        // 7. اختيار لون بعد كرت Wild (Choose Color) - FIXED & BULLETPROOF
        if (type === 'choose_color') {
            if (!room || room.status !== 'playing' || !room.pendingWild) return;

            // Safe match: checks playerSeat OR sessionId
            const isMatch = (room.pendingWild.playerSeat === player.seatIndex) || 
                            (room.pendingWild.sessionId === player.sessionId);
            if (!isMatch) return sendError(ws, 'ليس دورك لاختيار لون كرت Wild');

            const { chosenColor } = msg;
            const validColors = ['red', 'blue', 'green', 'yellow'];
            if (!chosenColor || !validColors.includes(chosenColor)) {
                return sendError(ws, 'اللون المختار غير صالح');
            }
    ...
    function handleClientDisconnect(roomCode, sessionId) {
        ...
        if (room.status === 'playing') {
            ...
            // If disconnected during player turn, trigger bot fallback after 3 seconds
...
// -------------------------------------------------------------
// Game Logic Execution (Server-Authoritative)
// -------------------------------------------------------------
function startRoomGame(room) {
    ...
    room.players.forEach(p => {
        p.cards = [];
        for (let i = 0; i < 7; i++) {
            p.cards.push(room.drawCard());
        }
    });

    // Draw first non-wild card for top of discard pile
    let firstCard = room.drawCard();
    while (firstCard.color === 'wild') {
        room.deck.unshift(firstCard);
        shuffle(room.deck);
        firstCard = room.drawCard();
    }

    room.currentTopCard = firstCard;
    room.activeColor = firstCard.color;
    room.discardPile = [firstCard];

    // Handle special initial card effects per UNO rules
    if (firstCard.type === 'reverse') {
        if (room.players.length === 2) {
            room.currentTurn = 1; // 2 players: reverse skips first player
        } else {
            room.turnDirection = -1;
        ...
        p0.cards.push(room.drawCard(), room.drawCard());
        room.currentTurn = 1;
    }
...
function executeCardDraw(room, player) {
    ...
    const prevTurn = room.currentTurn;
    const total = room.players.length;
    room.currentTurn = (room.currentTurn + room.turnDirection + total * 4) % total;
...
server.listen(PORT, () => {
    const localIPs = getLocalIPs();
    const primary = localIPs.find(ip => !ip.isVirtual) || localIPs[0];
    console.log('====================================================');
    console.log('🎮 سيرفر أونو 3D المحلي (UNO 3D LAN Server) يعمل بنجاح!');
    console.log('====================================================');
    console.log('🌐 محلياً على هذا الجهاز: http://localhost:' + PORT);
    if (primary) {
        console.log('👉 العنوان الموصى به للواي فاي (Wi-Fi LAN IP): http://' + primary.address + ':' + PORT);
    }
    if (localIPs.length > 1) {
        console.log('📋 عناوين الشبكة المكتشفة:');
        localIPs.forEach(ip => {
            const tag = ip.isVirtual ? ' [افتراضي/Virtual - تجنبه]' : (ip.priority >= 90 ? ' [Wi-Fi موصى به ⭐]' : '');
